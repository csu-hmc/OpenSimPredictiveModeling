function [motionData, osimState] = integrateOpenSimPlant(osimModel, initialState,...
...
numVar = initialState.getY().size();

initStateValues = zeros(numVar,1);
for i = 0:1:numVar-1
    initStateValues(i+1,1) = initialState.getY().get(i);
end
...
% passed as arguments
plantHandle = @(t,x) plantFunctionOpenSim(t, x, osimModel, initialState);

% Integrate the system equations
integratorFunc = str2func(integratorName);
[T, Y] = integratorFunc(plantHandle, timeSpan, initStateValues, ...
    integratorOptions);
